# Backup Monolithic Skill

This file preserves the previous monolithic `code-correctness` skill text as a fallback reference.

The active top-level skill now acts as the orchestrator for the modular lesson tree.

The old `lesson-guide/` entrypoint was removed. Use the main `SKILL.md` as the router for the lesson modules.

Use this backup only when:

- the task spans many themes at once and the lesson modules would be too fragmented,
- you want the older all-in-one working style for comparison,
- or you need to recover guidance that has not yet been redistributed into the modular lesson tree.

The original skill content starts below.
...
This skill now ships with a reel-based lesson tree extracted from `s4.codes`.

- Use `lesson-guide/SKILL.md` to navigate the curriculum by theme.
- Use the theme folders when you want the original lesson wording for a specific naming, function, comment, formatting, or object/data topic.
- Keep using this top-level skill as the main monolithic working skill; the lesson tree is supporting material, not a replacement.

## Ground the advice in the lessons

The lesson tree exists so your guidance stays anchored to real principles instead of generic cleanup language.

- When reviewing or refactoring, explain why a change matters in terms of the lesson it comes from, not just the mechanic of the change.
- If a recommendation clearly matches a lesson, name it in plain language and, when useful, mention the theme and lesson title directly. For example: `Meaningful Names / Avoid Disinformation`, `Functions / Do One Thing`, `Comments / Misleading and Confusing`, `Formatting / Code Reading Flow`, or `Objects and Data / Data / Object Anti-Symmetry`.
- Use the lesson to explain the reasoning behind the suggestion. Do not cite lessons as slogans or decorative labels.
- Do not force a lesson reference into every sentence. Use them when they help the reader understand the principle behind the edit or finding.
- When multiple lessons could apply, choose the one that best explains the core problem. If there is a real tradeoff, say so.
- If the task is tightly focused on one lesson topic, read the corresponding lesson module before answering so your explanation can stay close to the original transcript wording.
- Preserve restraint. A lesson reference is not permission to refactor code that is already clear and honest.

Good example:

- `Rename this variable because the current name is disinformation: it says list, but the value is actually a map. This matches Meaningful Names / Avoid Disinformation.`

Bad example:

- `Clean this up because clean code says so.`
